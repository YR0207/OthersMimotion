...
class MiMotion():
    ...
    def __init__(self, user, password, factor=1, cache_file=None):
    ...
    def process_user(user):
        ...
        # 其他情况 → 保持原样，third_name = "huami_phone"
            ...
            user = user
            third_name = "huami_phone"
        return user, third_name

    def login(self):

        """返回 (login_token, user_id, app_token) 任意一步失败返回 (0, 0, msg)"""
        # ---------- 阶段 1：拿 code ----------
        code_url = f"https://api-user.zepp.com/registrations/{self.user}/tokens"  # 去空格
        headers = {
        ...
        try:
            code_rsp = requests.post(code_url, data=code_data, headers=headers, timeout=10, allow_redirects=False)
            if code_rsp.status_code != 200:
                print(f"[登录阶段1] 状态码={code_rsp.status_code} 响应={code_rsp.text}")
                return 0, 0, 0
            code = code_rsp.json()["access"]
        ...
        # ---------- 阶段 2：拿 token ----------
        token_url = "https://account.zepp.com/v2/client/login"
        if "@" in self.user:
            token_data = {
                "app_name": "com.xiaomi.hm.health",
                "country_code": "CN",
        ...
        try:
            token_rsp = requests.post(token_url, data=token_data, headers=headers, timeout=10)
            if token_rsp.status_code != 200:
                print(f"[登录阶段2] 状态码={token_rsp.status_code} 响应={token_rsp.text}")
                return 0, 0, 0
            info = token_rsp.json()["token_info"]
    ...
    def _load_token_cache(self):
        """从缓存文件加载当前用户的 token，无缓存返回 None"""
        if not os.path.exists(self.cache_file):
            return None
        try:
            with open(self.cache_file, "r", encoding="utf-8") as f:
                cache = json.load(f)
            entry = cache.get(self.user)
            if entry and entry.get("app_token") and entry.get("user_id"):
                return entry
            return None
    ...
    def _save_token_cache(self, login_token, userid, app_token):
        """将 token 保存到缓存文件（合并写入，不影响其他账号）"""
        cache = {}
        ...
        try:
            ...
            print(f"[缓存] token 已保存")
        except IOError as e:
            print(f"[缓存] 保存失败: {e}")

    @staticmethod
    def _is_token_invalid(resp):
        """判断响应是否表明 token 已失效"""
        ...
        try:
            ...
            if any(kw in msg for kw in ("token", "unauthorized", "expired", "invalid")):
                return True
        except Exception:
            pass
        return False
    ...
    def get_step_by_time(self):
        ...
        min_step = int(26666)    # int(3500 * min_ratio * self.factor)
        max_step = int(29999)    # int(3500 * max_ratio * self.factor)
        return min_step, max_step

    def run(self):
        # ---------- 1. 计算步数范围 ----------
        try:
            min_step, max_step = self.get_step_by_time()
        ...
        # ---------- 2. 尝试使用缓存 token ----------
        userid = app_token = None
        cached = self._load_token_cache()
        if cached:
            userid = cached.get("user_id")
            app_token = cached.get("app_token")
            if userid and app_token:
                print("[缓存] 找到缓存 token，尝试直接提交")

        # ---------- 3. 无缓存则登录 ----------
        if not (userid and app_token):
            print("[缓存] 无可用缓存 token，开始登录...")
            login_token, userid, app_token = self.login()
            if 0 in (login_token, userid, app_token):
                msg = f"帐号信息: {self.user[3:7]}****{self.user[-4:]}\n修改信息: 登录失败\n"
                print(msg)
                return msg
            self._save_token_cache(login_token, userid, app_token)
        ...
        try:
            ...
            if resp.status_code != 200:
                print(f'[提交步数] 状态码={resp.status_code} 响应={resp.text}')
...
def get_factor_by_weather(area):
    if area == "NO":
        ...
        factor_dict = {"多云": 0.9, "阴": 0.8, "小雨": 0.7, "中雨": 0.5, "大雨": 0.4, "暴雨": 0.3, "大暴雨": 0.2, "特大暴雨": 0.2}
